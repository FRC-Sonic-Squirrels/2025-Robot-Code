...
public class SnapToReef extends Command {
  ...
  private static final LoggedTunableNumber xTolerance = group.build("xTolerance", 0.2);
  ...
  private static final LoggedTunableNumber yTolerance = group.build("yTolerance", 1);
  ...
  public SnapToReef(
      Supplier<TagOffset> tagOffset, DrivetrainWrapper wrapper, Supplier<Distance> distToWall) {
    this.tagOffset = tagOffset;
    this.wrapper = wrapper;
    this.distToWall = distToWall;

    horizPID.setTolerance(xTolerance.get());
    vertPID.setTolerance(yTolerance.get());
  ...
  public void execute() {
    ...
    if (xTolerance.hasChanged(hc)) horizPID.setTolerance(xTolerance.get());
    if (yTolerance.hasChanged(hc)) vertPID.setTolerance(yTolerance.get());
    ...
    wrapper.setVelocityOverride(
        new ChassisSpeeds(
            ...
            -horizPID.calculate(tagOffset.get().offset().getX(), 0),
            0));
  }
  ...
  public boolean isFinished() {
    return horizPID.atSetpoint() && vertPID.atSetpoint();
  }
}
