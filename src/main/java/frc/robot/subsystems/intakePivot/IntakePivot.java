...
public class IntakePivot extends SubsystemBase {
  ...
  private static final ExecutionTiming timing =
      new ExecutionTiming(IntakePivotConstants.ROOT_TABLE);

  // Logging
  private static final LoggerGroup logGroup = LoggerGroup.build(IntakePivotConstants.ROOT_TABLE);
  ...
  private static final TunableNumberGroup group =
      new TunableNumberGroup(IntakePivotConstants.ROOT_TABLE);
  ...
  private final PivotIO.Inputs inputs = new PivotIO.Inputs(logGroup);

  private ControlMode controlMode = ControlMode.OPEN_LOOP;
  private Rotation2d targetAngleDegrees = Constants.zeroRotation2d;

  /** Creates a new IntakePivotSubsytem. */
  public IntakePivot(PivotIO io) {
    this.io = io;
  ...
  public void periodic() {
    try (var ignored = timing.start()) {
      ...
      logInputs_angle.info(inputs.pivotPosition);
      logInputs_appliedVolts.info(inputs.pivotAppliedVolts);
      logInputs_currentAmps.info(inputs.pivotCurrentAmps);
      logInputs_tempCelsius.info(inputs.pivotTempCelsius);
      logInputs_velocityDegreesPerSecond.info(inputs.pivotVelocityDegreesPerSecond);
  ...
  public void resetSensorToHomePosition() {
    io.resetSensorPosition(Constants.IntakePivotConstants.MIN_PIVOT_ANGLE);
  }
