...
public class PivotIOReal implements PivotIO {
  ...
  public void updateInputs(Inputs inputs) {
    ...
    inputs.pivotPosition = Rotation2d.fromRotations(position.getValue().in(Units.Rotations));
    inputs.pivotAppliedVolts = appliedVoltage.getValue().in(Units.Volts);
    inputs.pivotCurrentAmps = current.getValue().in(Units.Amps);
    inputs.pivotTempCelsius = temp.getValue().in(Units.Celsius);
    inputs.pivotVelocityDegreesPerSecond = velocity.getValue().in(Units.DegreesPerSecond);
  }
